<script lang="ts">
  ...
  function updateSelectors() {
    if (!$worktopOciswap.coins.has(sendFungibleAddress)) {
      sendFungibleAddress = "";
    }
    if (sendFungibleAddress === "" && $worktopOciswap.coins.size > 0) {
      sendFungibleAddress = $worktopOciswap.coins.keys().next().value;
    }
