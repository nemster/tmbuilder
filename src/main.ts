...
import {radixplanet_pools, radixplanet_listed_coins, radixplanet_lp} from './radixplanet.ts'
...
const gable_transient_nft= "resource_rdx1ngxzt7uq9l2wm5gd8vefcq5pkwcqwrn530a98p72mnkjzjev8hlxdn";
const gable_lsu= "resource_rdx1thrz4g8g83802lumrtrdsrhjd6k5uxhxhgkrwjg0jn75cvxfc99nap";
const gable_liquidity_nft= "resource_rdx1nfxg3t4eyls2qycqqrp6df8wkz3n2r04ex20443jtsgz5c23wsf74w";
var non_fungibles_symbols: {[key: string]: string}= {
  ...
  "resource_rdx1nt3vrt8xtdal6gn7ddv0zfzvxpqylxyfmr97setz8r3amhhk90yqmg": "Weft Claimer Nft"
};
...
document.querySelector<HTMLSelectElement>('#action')!.addEventListener("change", function() {
  ...
  document.querySelector<HTMLDivElement>('#div18')!.hidden= (this.selectedIndex != 20); //provide liquidity to Gable
  ...
  } else if (this.selectedIndex == 20 && (fungibles_in_worktop[gable_lsu] == undefined || fungibles_in_worktop[gable_lsu] == 0)) {
    document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "put some Gable LSUs in the worktop first";
  }

  // --- DEFIPLAZA ---
  document.querySelector<HTMLDivElement>('#div16')!.hidden= (this.selectedIndex != 22); //swap coins at DefiPlaza
  if ((this.selectedIndex == 22) && document.querySelector<HTMLSelectElement>('#send16')!.options.length == 0 ) {
    document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "put some coin listed on DefiPlaza in the worktop first";
  }

  // --- ALPHADEX ---
  document.querySelector<HTMLDivElement>('#div17')!.hidden= (this.selectedIndex != 24); //swap coins at AlphaDEX
  if ((this.selectedIndex == 24) && document.querySelector<HTMLSelectElement>('#send17')!.options.length == 0 ) {
    document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "put some coin listed on AlphaDEX in the worktop first";
  }

  // --- RADIXPLANET ---
  document.querySelector<HTMLDivElement>('#div19')!.hidden= (this.selectedIndex != 26); //swap coins at RadixPlanet
  if ((this.selectedIndex == 26) && document.querySelector<HTMLSelectElement>('#send19')!.options.length == 0 ) {
    document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "put some coin listed on RadixPlanet in the worktop first";
  }

  // --- WEFT ---
  document.querySelector<HTMLDivElement>('#div20')!.hidden= (this.selectedIndex != 28); //redeem your WEFT
  if ((this.selectedIndex == 28) && document.querySelector<HTMLSelectElement>('#nft20')!.options.length == 0 ) {
    document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "you don't have a Weft Claimer NFT";
  }
});
...
document.querySelector<HTMLButtonElement>('#add_instruction15')!.addEventListener("click", function() {
  ...
  remove_non_fungible_from_worktop(gable_transient_nft + ' ' + unknown_nft_id);
});
...
document.querySelector<HTMLInputElement>('#send17')!.addEventListener("change", function() {
  const send17= document.querySelector<HTMLSelectElement>('#send17')!.value;
  ...
  if (!document.querySelector<HTMLInputElement>('#all17')!.checked) {
    document.querySelector<HTMLInputElement>('#quantity17')!.value= String(fungibles_in_worktop[send17]);
  }
  receive17!.innerHTML= "";
  for (var receive of Object.keys(alphadex_listed_coins)) {
    if (receive != send17) {
      receive17!.options[receive17!.options.length]= new Option(find_fungible_symbol(receive), receive);
    }
  }
});

document.querySelector<HTMLInputElement>('#all17')!.addEventListener("change", function() {
  if (!document.querySelector<HTMLInputElement>('#all17')!.checked) {
    document.querySelector<HTMLInputElement>('#quantity17')!.value= String(fungibles_in_worktop[document.querySelector<HTMLSelectElement>('#send17')!.value]);
    document.querySelector<HTMLInputElement>('#quantity17')!.disabled= false;
  } else {
    document.querySelector<HTMLInputElement>('#quantity17')!.value= '';
    document.querySelector<HTMLInputElement>('#quantity17')!.disabled= true;
  }
});

document.querySelector<HTMLButtonElement>('#add_instruction17')!.addEventListener("click", async function() {
  document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "&nbsp;";
  ...
  if (document.querySelector<HTMLInputElement>('#all17')!.checked) {
    ...
    const q= document.querySelector<HTMLSelectElement>('#quantity17')!.value;
    if (!q.match(/^[0-9]+(\.[0-9]+)?$/)) {
      document.querySelector<HTMLInputElement>('#warn')!.innerText= "invalid quantity!";
      return false;
    }
...
document.querySelector<HTMLInputElement>('#all18')!.addEventListener("change", function() {
  if (!document.querySelector<HTMLInputElement>('#all18')!.checked) {
    if (fungibles_in_worktop[gable_lsu] != undefined) {
      document.querySelector<HTMLInputElement>('#quantity18')!.value= String(fungibles_in_worktop[gable_lsu]);
    }
    document.querySelector<HTMLInputElement>('#quantity18')!.disabled= false;
  } else {
    document.querySelector<HTMLInputElement>('#quantity18')!.value= '';
    document.querySelector<HTMLInputElement>('#quantity18')!.disabled= true;
  }
});

document.querySelector<HTMLButtonElement>('#add_instruction18')!.addEventListener("click", async function() {
  document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "&nbsp;";
  ...
  if (document.querySelector<HTMLInputElement>('#all18')!.checked) {
    ...
    const q= document.querySelector<HTMLSelectElement>('#quantity18')!.value;
    if (!q.match(/^[0-9]+(\.[0-9]+)?$/)) {
      document.querySelector<HTMLInputElement>('#warn')!.innerText= "invalid quantity!";
      return false;
    }
...
document.querySelector<HTMLInputElement>('#send19')!.addEventListener("change", function() {
  const send19= document.querySelector<HTMLSelectElement>('#send19')!.value;
  ...
  if (!document.querySelector<HTMLInputElement>('#all19')!.checked) {
    document.querySelector<HTMLInputElement>('#quantity19')!.value= String(fungibles_in_worktop[send19]);
  }
  receive19!.innerHTML= "";
  for (var pool of Object.keys(radixplanet_pools)) {
    ...
    if (found) {
      for (var resource of radixplanet_pools[pool].resources) {
        if (resource != send19) {
          receive19!.options[receive19!.options.length]= new Option(find_fungible_symbol(resource) + ' - ' + radixplanet_pools[pool].name + ' pool', resource + ' ' + pool);
	}
      }
    }
  }
...
document.querySelector<HTMLInputElement>('#all19')!.addEventListener("change", function() {
  if (!document.querySelector<HTMLInputElement>('#all19')!.checked) {
    const send19= document.querySelector<HTMLSelectElement>('#send19')!.value;
    document.querySelector<HTMLInputElement>('#quantity19')!.value= String(fungibles_in_worktop[send19]);
    document.querySelector<HTMLInputElement>('#quantity19')!.disabled= false;
  } else {
    document.querySelector<HTMLInputElement>('#quantity19')!.value= '';
    document.querySelector<HTMLInputElement>('#quantity19')!.disabled= true;
  }
});

document.querySelector<HTMLButtonElement>('#add_instruction19')!.addEventListener("click", async function() {
  document.querySelector<HTMLParagraphElement>('#warn')!.innerHTML= "&nbsp;";

  const send19= document.querySelector<HTMLSelectElement>('#send19')!.value;
  const receive19= document.querySelector<HTMLSelectElement>('#receive19')!.value;
  if (send19 == undefined || receive19 == undefined) {
    document.querySelector<HTMLInputElement>('#warn')!.innerText= "select coins to swap!";
  }
  ...
  if (document.querySelector<HTMLInputElement>('#all19')!.checked) {
    ...
    const q= document.querySelector<HTMLSelectElement>('#quantity19')!.value;
    if (!q.match(/^[0-9]+(\.[0-9]+)?$/)) {
      document.querySelector<HTMLInputElement>('#warn')!.innerText= "invalid quantity!";
      return false;
    }
  ...
  gatewayApi.state.getEntityDetailsVaultAggregated([r[1]]).then((response) => {
    ...
    var received_quantity= receive_liquidity * (1 - (send_liquidity / (send_liquidity + quantity19 * (1 - fees))));
...
document.querySelector<HTMLButtonElement>('#add_instruction20')!.addEventListener("click", function() {
